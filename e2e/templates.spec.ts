...
test("plantilla: s'afegeix, s'edita, es carrega a Avui, es marquen sèries i en acabar s'actualitza", async ({ page }) => {
  ...
  const weight1 = page.getByRole("textbox", { name: "Pes en kg, Pressió sobre banc, sèrie 1" });
  ...
  await page.getByRole("button", { name: "Marca la sèrie 1 de Pressió sobre banc", exact: true }).click();
  await expect(page.getByText(/^Començat a les/)).toBeVisible();

  const weight2 = page.getByRole("textbox", { name: "Pes en kg, Pressió sobre banc, sèrie 2" });
  await weight2.fill("65");
  await page.getByRole("button", { name: "Marca la sèrie 2 de Pressió sobre banc", exact: true }).click();
  await expect(page.getByRole("button", { name: "Desmarca la sèrie 2 de Pressió sobre banc", exact: true })).toBeVisible();
  await page.getByRole("button", { name: "Marca la sèrie 1 de Pressió d'espatlles" }).click();

  // Desmarcar i tornar a marcar funciona.
  await page.getByRole("button", { name: "Desmarca la sèrie 1 de Pressió d'espatlles" }).click();
  await page.getByRole("button", { name: "Marca la sèrie 1 de Pressió d'espatlles" }).click();
  ...
  await expect(page.locator("article", { hasText: "Pressió sobre banc" })).toContainText("65 kg × 6 reps");
  await expect(page.locator("article", { hasText: "Pressió d'espatlles" })).toBeVisible();

  // La plantilla té els pesos nous.
  await page.getByRole("button", { name: "Ajustos", exact: true }).click();
  await page.getByRole("button", { name: /^Pit, espatlles i tríceps/ }).click();
  await expect(page.getByRole("textbox", { name: "Pes en kg, Pressió sobre banc, sèrie 1" })).toHaveValue("62,5");
  await expect(page.getByRole("textbox", { name: "Pes en kg, Pressió sobre banc, sèrie 2" })).toHaveValue("65");
...
test("les plantilles s'importen d'un fitxer JSON", async ({ page }) => {
  ...
  await page.getByLabel("Fitxer de plantilles").setInputFiles({
  ...
  // Noms antics normalitzats al català actual (TERMCAT).
  await expect(page.getByRole("heading", { level: 3, name: "Pes mort" })).toBeVisible();
  await expect(page.getByRole("heading", { level: 3, name: "Planxa amb quatre suports" })).toBeVisible();
});
