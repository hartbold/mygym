...
test("crea un exercici amb sèries de pesos diferents, l'acaba en prémer «Nou exercici», acaba explícitament, agrupa per dia i persisteix", async ({
  ...
  await page.getByRole("button", { name: "Press banca", exact: true }).click();
  ...
  const pressCard = page.locator("article", { hasText: "Press banca" });
  ...
  await expect(page.locator("article", { hasText: "Press banca" })).toBeVisible();
